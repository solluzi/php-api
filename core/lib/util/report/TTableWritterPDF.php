...
class TTableWriterPDF implements ITableWriter
{
    ...
    {
        $this->styles[$stylename] = [$fontface, $fontsize, $fontstyle, $fontcolor, $fontcolor, $fillcolor];
    }
     ...
     * @param $colspan   quantidade de células a serem mesclados
     */
    public function addCell($content, $align, $stylename = null, $colspan = 1)
    {
        ...
        $width = 0;
        // calcula a largura da célula(incluindo as mecladas)
        for ($n = $this->colcounter; $n < $this->colcounter + $colspan; $n++) {
            $width += $this->widths[$n];
        }
        // exibe a celula com o conteudo passado
        $this->pdf->Cell($width, $fontsize * 1.5, $content, 1, 0, strtoupper(substr($align, 0, 1)), true);
        $this->colcounter++;
    }

    /**
     * Armazena o conteúdo do documento em arquivo
